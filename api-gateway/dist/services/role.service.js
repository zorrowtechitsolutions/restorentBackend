...
const callRoleService = async (options) => {
    return (0, httpClient_1.httpClient)(options);
};
const breaker = new opossum_1.default(callRoleService, {
    timeout: 50000,
    errorThresholdPercentage: 50,
    resetTimeout: 15000,
});
breaker.fallback(() => {
    return { status: 503, data: { success: false, message: "Role service temporarily unavailable" } };
});
const proxyRequest = async (req, res, next) => {
    try {
        const url = `${services_1.SERVICES.ROLE_SERVICE}${req.originalUrl.replace("/api/roles", "/roles").replace("/api/permissions", "/permissions")}`;
        const options = {
            method: req.method,
            url: url,
            data: req.body,
