...
const breaker = new opossum_1.default(callUserService, {
    timeout: 50000,
...
const proxyRequest = async (req, res, next) => {
    try {
        const url = `${services_1.SERVICES.USER_SERVICE}${req.originalUrl.replace("/api/users", "/users").replace("/api/patients", "/patients").replace("/api/prescription", "/prescription").replace("/api/vitals", "/vitals").replace("/api/documents", "/documents").replace("/api/lab-results", "/lab-results").replace("/api/email-enquiry", "/email-enquiry")}`;
        const options = {
            method: req.method,
            url: url,
            data: req.body,
