...
const callCompanyService = async (options: any) => {
  return httpClient(options);
};

const breaker = new CircuitBreaker(callCompanyService, {
  timeout: 50000,
...
export const proxyRequest = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const url = `${SERVICES.COMPANY_SERVICE}${req.originalUrl.replace("/api/", "/")}`;
    const options = {
      method: req.method,
      url: url,
      data: req.body,
