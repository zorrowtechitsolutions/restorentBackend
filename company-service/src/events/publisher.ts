    ...
    while (!connected && attempts < maxAttempts) {
        try {
            ...
            connection.on('error', (err) => {
                logger.error('❌ Company Service RabbitMQ Connection Error:', { error: err.message });
            });

            connection.on('close', () => {
                logger.warn('⚠️ Company Service RabbitMQ Connection closed. Retrying...');
                channel = null as any;
                setTimeout(connectRabbitMQ, 5000);
            });

            channel = await connection.createChannel();
            logger.info('🐰 Company Service connected to RabbitMQ');
            connected = true;
        } catch (error: any) {
            attempts++;
