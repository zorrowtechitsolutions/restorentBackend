...
import notificationAlertRoutes from "./routes/notification-alert.routes";
...
app.use("/", notificationAlertRoutes);
...
app.use(
    (
        ...
        res.status(404).json({
            status: 404,
            message: "Requested company-related resource not found",
        });
    }
);
...
app.use(
    (
        ...
        res.status(err.status || 500).json({
            success: false,
            message: err.message || "Internal Server Error in Company Service",
            error: {
                message: err.message,
